...
import { GameState, ServerToClientEvents, ClientToServerEvents, Card, Player } from '@tongits/shared';
import { createDeck, shuffleDeck, calculatePoints, isValidMeld, sortMeld } from './gameLogic';
...
function broadcastGameState() {
  ...
  io.sockets.sockets.forEach((socket) => {
    ...
    sanitizedState.players.forEach(p => {
      p.points = calculatePoints(p.hand);
      if (p.id !== receiverId) {
        // Map hand to hidden cards (so client knows count but not actual cards)
        p.hand = p.hand.map(c => ({ id: 'hidden', suit: 'hearts', rank: 'A', value: 0 } as Card));
      }
    });
...
function endRound(winnerId: string, reason: string) {
...
function evaluateFight() {
  ...
  const eligiblePlayers: { id: string, points: number }[] = [];

  gameState.players.forEach(p => {
    let pPoints = calculatePoints(p.hand);
...
io.on('connection', (socket) => {
  ...
  socket.on('drawCard', () => {
    const player = gameState.players.find(p => p.socketId === socket.id);
    if (!player || gameState.turnId !== player.id || gameState.hasDrawnThisTurn || gameState.status !== 'playing') return;
  ...
  socket.on('pickDump', (targetMeldIndices: number[]) => {
    const player = gameState.players.find(p => p.socketId === socket.id);
    if (!player || gameState.turnId !== player.id || gameState.hasDrawnThisTurn || gameState.status !== 'playing') return;
    if (gameState.dumpPile.length === 0) return;
    ...
    if (isValidMeld(proposedMeldCards)) {
      turnHistory = []; // Pick dump cannot be undone
      const sortedMeld = sortMeld(proposedMeldCards);
  ...
  socket.on('layMeld', (cardIndices: number[]) => {
    const player = gameState.players.find(p => p.socketId === socket.id);
    if (!player || gameState.turnId !== player.id || !gameState.hasDrawnThisTurn || gameState.status !== 'playing') return;

    const proposedCards = cardIndices.map(i => player.hand[i]);
    if (isValidMeld(proposedCards)) {
      saveStateSnapshot();
      const sortedMeld = sortMeld(proposedCards);
      // Remove from hand
  ...
  socket.on('sapaw', (cardIndices: number[], targetPlayerId: string, meldIndex: number) => {
    const player = gameState.players.find(p => p.socketId === socket.id);
    if (!player || gameState.turnId !== player.id || !gameState.hasDrawnThisTurn || gameState.status !== 'playing') return;
    ...
    if (isValidMeld(combinedMeld)) {
      saveStateSnapshot();
      const sortedMeld = sortMeld(combinedMeld);
      // Remove from hand
      const cardsToRemove = new Set(proposedCards.map(c => c.id));
      player.hand = player.hand.filter(c => !cardsToRemove.has(c.id));
  ...
  socket.on('respondToFight', (response: 'fold' | 'challenge') => {
    const player = gameState.players.find(p => p.socketId === socket.id);
    if (!player || gameState.phase !== 'fight_challenge' || !gameState.fightResponses) return;
    if (player.id === gameState.fightCallerId) return; // Caller cannot respond
    if (gameState.fightResponses[player.id]) return; // Already responded
  ...
  socket.on('undo', () => {
    const player = gameState.players.find(p => p.socketId === socket.id);
    if (!player || gameState.turnId !== player.id || gameState.status !== 'playing') return;
